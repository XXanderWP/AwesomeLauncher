...
function elybyJavaAgentArg() {
  ...
  // No quoting needed: spawn() passes args as an array, so spaces in the path
  // are handled by the OS directly. Adding quotes would make them literal
  // characters in the path, causing "JAR manifest missing" on paths like
  // "C:\Program Files\...".
  return `-javaagent:${jar}=ely.by`
}
...
class ProcessBuilder {
  ...
  build() {
    ...
    this.usingNeoForgeLoader =
      this.server.modules.some((mdl) => {
        ...
        return mdl.subModules.some(
          (sub) =>
            typeof sub.rawModule.id === 'string' &&
            sub.rawModule.id.startsWith('net.neoforged:neoforge:')
        )
      }) ||
  ...
  constructJSONModList(type, mods, save = false) {
    const modList = {
      repositoryRoot:
        ...
        this._getForgeLikeStoreAbsolute()
    }
  ...
  _constructJVMArguments112(mods, tempNativePath) {
    ...
    if (process.platform === 'darwin') {
      args.push('-Xdock:name=AwesomeCraftLauncher')
        ...
        if (argDiscovery.test(args[i])) {
          ...
          switch (identifier) {
            ...
            case 'version_name':
              ...
              val = this.usingNeoForgeLoader ? this.modManifest.id : this.server.rawServer.id
              break
            case 'game_directory':
              val = this.gameDir
              break
    ...
    for (let i = 0; i < libArr.length; i++) {
      ...
      if (isLibraryCompatible(lib.rules, lib.natives)) {
        ...
        // 1.19+ logic
          ...
          // Extract the native library.
          const exclusionArr =
            lib.extract != null ? lib.extract.exclude : ['META-INF/', '.git', '.sha1']
